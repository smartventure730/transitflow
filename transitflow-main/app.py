...
def init_db():
    ...
    try:
        ...
        # ==========================================================
        # NEW SCHEMA UPDATE: DAILY ROLLING LOCKS MATRIX
        # ==========================================================
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS daily_locked_profits (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id INTEGER,
                purchase_id INTEGER,
                amount REAL,
                earned_at TIMESTAMP,
                unlock_at TIMESTAMP,
                status TEXT DEFAULT 'Locked'  -- 'Locked' or 'Unlocked'
            )
        ''')
        ...
        conn.commit()
        print("[Database] Structural integrity validation verified completely.")
...
def process_daily_profits(user_id):
    """
    Tracks precise 24-hour periods from the exact time of deployment.
    Logs daily rewards into a 14-day lock section and matures funds 
    automatically onto the withdrawable dashboard when the lock window ends.
    """
    conn = get_db_connection()
    cursor = conn.cursor()
    ...
    try:
        # -------------------------------------------------------------
        # ROUTINE 1: ACCRUE INDIVIDUAL DAILY RETURN CYCLES
        # -------------------------------------------------------------
        ...
        for p in active_purchases:
            expires_at = datetime.strptime(p['expires_at'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=uganda_tz)
            
            if now >= expires_at:
                cursor.execute("UPDATE purchases SET status = 'Completed' WHERE id = ?", (p['id'],))
                continue
                
            last_payout_str = p['last_payout_date']
            # Parse the last tracking time (handles legacy calendar date text gracefully via fallback)
            if last_payout_str and len(last_payout_str) > 10:
                last_payout_time = datetime.strptime(last_payout_str, '%Y-%m-%d %H:%M:%S').replace(tzinfo=uganda_tz)
            else:
                last_payout_time = datetime.strptime(p['activated_at'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=uganda_tz)
                
            # Check if an exact 24-hour window has rolled over
            if now >= last_payout_time + timedelta(hours=24):
                hours_passed = (now - last_payout_time).total_seconds() / 3600
                days_due = int(hours_passed // 24)
                
                if days_due > 0:
                    daily_amt = p['daily_return'] * days_due
                    earned_time_str = now.strftime('%Y-%m-%d %H:%M:%S')
                    unlock_time_str = (now + timedelta(days=14)).strftime('%Y-%m-%d %H:%M:%S')
                    
                    # 1. Update overall metadata profit records
                    cursor.execute('''
                        UPDATE users 
                        SET cumulative_profit = cumulative_profit + ? 
                        WHERE id = ?
                    ''', (daily_amt, user_id))
                    
                    # 2. Place this return stream in isolation inside our rolling ledger
                    cursor.execute('''
                        INSERT INTO daily_locked_profits (user_id, purchase_id, amount, earned_at, unlock_at, status)
                        VALUES (?, ?, ?, ?, ?, 'Locked')
                    ''', (user_id, p['id'], daily_amt, earned_time_str, unlock_time_str))
                    
                    # 3. Append to system audit trail
                    cursor.execute('''
                        INSERT INTO transaction_archive (tx_type, amount, status, notes, processed_at)
                        VALUES ('Daily Return Locked', ?, 'Success', ?, ?)
                    ''', (daily_amt, f"Daily yield for {p['tier_name']} locked for 14-days.", earned_time_str))
                    
                    # 4. Advance tracking timeline markers
                    new_payout_checkpoint = last_payout_time + timedelta(days=days_due)
                    cursor.execute(
                        "UPDATE purchases SET last_payout_date = ? WHERE id = ?", 
                        (new_payout_checkpoint.strftime('%Y-%m-%d %H:%M:%S'), p['id'])
                    )

        # -------------------------------------------------------------
        # ROUTINE 2: RELEASE 14-DAY COUNTER MATURED WALLETS
        # -------------------------------------------------------------
        matured_payouts = cursor.execute('''
            SELECT * FROM daily_locked_profits 
            WHERE user_id = ? AND status = 'Locked'
        ''', (user_id,)).fetchall()
        
        for row in matured_payouts:
            unlock_at = datetime.strptime(row['unlock_at'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=uganda_tz)
            
            if now >= unlock_at:
                # 1. Unleash funds to standard balance matrix
                cursor.execute('''
                    UPDATE users 
                    SET balance = balance + ? 
                    WHERE id = ?
                ...
                # 2. Mark ledger item as Unlocked
                cursor.execute(
                    "UPDATE daily_locked_profits SET status = 'Unlocked' WHERE id = ?", 
                    (row['id'],)
                )
                
                # 3. Log release to transaction archive
                cursor.execute('''
                    INSERT INTO transaction_archive (tx_type, amount, status, notes, processed_at)
                    VALUES ('Matured Payout Released', ?, 'Success', ?, ?)
                ''', (row['amount'], f"14-day lock cycle ended. Matured funds released.", now.strftime('%Y-%m-%d %H:%M:%S')))
                
        conn.commit()
    except Exception as e:
        conn.rollback()
        print(f"[Engine Error] Failed to process daily yields or roll over lock matrix: {e}")
        raise e
    finally:
        conn.close()
...
def dashboard():
    ...
    # --- FETCH LOCK LEDGER DATA FOR VIEWING ---
    cursor.execute("SELECT COALESCE(SUM(amount), 0.0) FROM daily_locked_profits WHERE user_id = ? AND status = 'Locked'", (session['user_id'],))
    total_locked_balance = cursor.fetchone()[0]
    
    cursor.execute("SELECT * FROM daily_locked_profits WHERE user_id = ? ORDER BY unlock_at ASC", (session['user_id'],))
    locked_profits_list = [dict(row) for row in cursor.fetchall()]
    # ---------------------------------------------
    ...
    uganda_tz = pytz.timezone('Africa/Kampala')
    now_uganda = datetime.now(uganda_tz)
    
    for p in all_purchases:
        act_date = datetime.strptime(p['activated_at'], '%Y-%m-%d %H:%M:%S').replace(tzinfo=uganda_tz)
        p['days_held'] = max(0, (now_uganda - act_date).days)
    ...
    return render_template_string(
        ...
        locked_profits_list=locked_profits_list
    )
...
def purchase():
    ...
    if user_bal >= cost:
        # Force the exact live Uganda time zone markers
        uganda_tz = pytz.timezone('Africa/Kampala')
        now_uganda = datetime.now(uganda_tz)
        
        activated_str = now_uganda.strftime('%Y-%m-%d %H:%M:%S')
        expiry_str = (now_uganda + timedelta(days=14)).strftime('%Y-%m-%d %H:%M:%S')
        
        # Initialize last_payout_date with the full timestamp of the exact purchase second
        last_payout_init_str = activated_str

        # Deduct balance from user
        cursor.execute("UPDATE users SET balance = balance - ? WHERE id = ?", (cost, session['user_id']))
        
        # Insert package subscription tracking explicit timestamp streams
        cursor.execute('''
            INSERT INTO purchases (user_id, tier_name, cost, daily_return, activated_at, expires_at, status, last_payout_date)
            VALUES (?, ?, ?, ?, ?, ?, 'Active', ?)
        ''', (session['user_id'], tier, cost, daily, activated_str, expiry_str, last_payout_init_str))
...
<div class="container mt-4 mb-5">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade" id="tab-history">
            <div class="card p-3 mb-3 bg-dark text-white" style="border: 1px solid #222943; border-radius: 8px;">
                <h5 class="fw-bold"><i class="fa-solid fa-vault text-warning me-2"></i>Escrow Holdings (Locked Profits)</h5>
                <h3 class="text-warning fw-bold mt-1">UGX {{ugx(total_locked_balance)}}</h3>
                <p class="small text-muted mb-3">Daily profits are held for 14 days from accrual before moving to available withdrawal balance.</p>
                
                <div class="table-responsive small">
                    <table class="table table-dark table-sm table-striped align-middle mb-0">
                        <thead>
                            <tr><th>Amount</th><th>Accrued On</th><th>Unlocks On</th><th>Status</th></tr>
                        </thead>
                        <tbody>
                            {% for item in locked_profits_list %}
                            <tr>
                                <td class="text-success fw-bold">UGX {{ugx(item.amount)}}</td>
                                <td>{{item.earned_at}}</td>
                                <td class="text-info">{{item.unlock_at}}</td>
                                <td><span class="badge bg-{{ 'secondary' if item.status == 'Locked' else 'success' }}">{{item.status}}</span></td>
                            </tr>
                            {% endfor %}
                            {% if not locked_profits_list %}
                            <tr>
                                <td colspan="4" class="text-center text-muted py-2">No locked operations or history entries running.</td>
                            </tr>
                            {% endif %}
                        </tbody>
                    </table>
            ...
            <h5 class="text-white fw-bold mb-3 mt-4">Live Active Deployments</h5>
